...
import { X, Image, FolderCheck, Sparkles, RefreshCw, AlertCircle } from 'lucide-react';
      ...
      <div className="w-full max-w-lg bg-slate-900 border border-white/10 rounded-2xl shadow-2xl flex flex-col max-h-[85vh] overflow-hidden">
        ...
        <div className="p-4 border-b border-white/10 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-base font-bold text-white">Google Photos Albums</h3>
              <p className="text-xs text-slate-400">
                Choose an album for your top slideshow
              </p>
            </div>
          </div>
        ...
        {/* Error notification if Google returned an error */}
        {photosError && (
          <div className="mx-4 mt-3 p-3 rounded-xl bg-amber-950/60 border border-amber-500/40 text-amber-200 text-xs flex items-start gap-2">
            ...
            <div className="space-y-1">
              ...
              <p className="text-[11px] text-amber-300">
                Tip: In Google Cloud Console, ensure <strong>Photos Library API</strong> is enabled and re-authenticate in Settings.
              </p>
            </div>
          </div>
        )}
        ...
        <div className="px-4 py-2.5 bg-slate-950/80 border-b border-white/5 space-y-1.5">
          <label className="text-[11px] font-semibold text-slate-400">
            Have a specific Google Photos Album ID or Link?
          </label>
        ...
        <div className="p-4 overflow-y-auto space-y-2 flex-1">
          {albums.length === 0 ? (
            <div className="py-12 text-center text-slate-400">
              ...
              <p className="text-sm font-semibold text-white">No albums loaded yet</p>
              <p className="text-xs text-slate-400 mt-1">
                {isGoogleConnected
                  ? 'Click the Refresh icon above to scan your Google Photos library.'
                  : 'Connect your Google account in Settings to see your Google Photos albums.'}
              </p>
            </div>
          ) : (
            albums.map((album) => {
              const isSelected = album.id === selectedAlbumId;
              const isLibrary = album.id === 'ALL_LIBRARY_PHOTOS';
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-slate-400 mt-0.5">
                      {isLibrary
                        ? 'Stream latest photos from your entire library'
